...
export function recommendationText(rec: Recommendation) {
  if (rec === 'BUY_NOW') return {
    ...
    desc: '현재 진행 중인 원화 환산 Dubai 지표가 직전 산정기간보다 높고, 선택 노선 기준 참고 영향액이 5만 원 이상입니다. 유류비 관점에서는 지금 발권이 유리할 수 있어요.'
  }
  if (rec === 'WAIT') return {
    title: '거리반영 참고액 기준, 기다리는 전략도 가능해요',
    desc: '현재 진행 중인 원화 환산 Dubai 지표가 직전 산정기간보다 낮고, 선택 노선 기준 참고 영향액이 5만 원 이상입니다. 일정이 급하지 않다면 기다리는 전략도 고려할 수 있어요.'
  }
