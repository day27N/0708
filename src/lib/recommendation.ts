...
export function recommendationText(rec: Recommendation) {
  if (rec === 'BUY_NOW') return {
    ...
    desc: '현재 진행 중인 원화 환산 Dubai 지표가 직전 산정기간보다 높고, 선택 노선 편도 기준 참고 영향액이 2.5만 원 이상입니다. 유류비 관점에서는 지금 발권이 유리할 수 있어요.'
  }
  if (rec === 'WAIT') return {
    title: '거리반영 참고액 기준, 기다리는 전략도 가능해요',
    desc: '현재 진행 중인 원화 환산 Dubai 지표가 직전 산정기간보다 낮아 비용이 내려가는 추세이고, 선택 노선 편도 기준 참고 영향액이 2.5만 원 이상입니다. 일정이 급하지 않다면 기다리는 전략도 고려할 수 있어요.'
  }
  if (rec === 'WEAK_SIGNAL') return {
    title: '방향성은 보이지만, 차이는 크지 않아요',
    desc: '원화 환산 Dubai 지표는 움직이고 있지만, 선택 노선 편도 기준 참고 영향액이 2.5만 원에는 미치지 않습니다. 유류비만으로 발권 시점을 결정하기보다는 항공권 기본 운임과 좌석 재고를 함께 확인하는 것이 좋아요.'
  }
