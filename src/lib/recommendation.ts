...
export function recommendationText(rec: Recommendation, deltaKrwPerBarrel: number | null = null) {
  if (rec === 'BUY_NOW') return {
    title: '유류할증료 상승 압력이 커요',
    desc: '현재 진행 중인 원화 환산 Dubai 지표가 직전 산정기간보다 높고, 선택 노선 편도 기준 참고 영향액이 2.5만 원 이상입니다. 지금 발권이 유리할 수 있어요.'
  }
  if (rec === 'WAIT') return {
    title: '유류할증료 하락 압력이 커요',
    desc: '현재 진행 중인 원화 환산 Dubai 지표가 직전 산정기간보다 낮고, 선택 노선 편도 기준 참고 영향액이 2.5만 원 이상입니다. 조금 기다려볼 만해요.'
  }
  if (rec === 'WEAK_SIGNAL' && deltaKrwPerBarrel !== null && deltaKrwPerBarrel > 0) return {
    title: '소폭 상승 가능성이 있어요',
    desc: '원화 환산 Dubai 지표는 오르는 방향이지만, 선택 노선 편도 기준 참고 영향액은 2.5만 원보다 작습니다. 유류비만으로 발권 시점을 결정하기보다는 항공권 기본 운임과 좌석 재고를 함께 확인하는 것이 좋아요.'
  }
  if (rec === 'WEAK_SIGNAL') return {
    title: '소폭 하락 가능성이 있어요',
    desc: '원화 환산 Dubai 지표는 내려가는 방향이지만, 선택 노선 편도 기준 참고 영향액은 2.5만 원보다 작습니다. 일정이 급하지 않다면 조금 더 지켜볼 수 있지만, 항공권 기본 운임과 좌석 재고를 함께 확인하는 것이 좋아요.'
  }
